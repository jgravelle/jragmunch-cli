...
class DoctorReport:
    ...
    def render(self) -> str:
        ...
        if self.auth_mode == "api":
            ...
            lines.append("auth mode:       subscription (default; OAuth login, billed against Anthropic's monthly Agent SDK credit. $0 until the credit is exhausted; see README for tier-scaled credit amounts and the credit-exhaustion contract.)")
        if self.error:
